...
func TestConcurrentFrequencyWO(t *testing.T) {
	seq := OriginalFrequency(euro + dutch + us)
	t.Log("Original: ", seq)
	con := ConcurrentFrequencyWO([]string{euro, dutch, us})
...
func TestConcurrentFrequencyWC(t *testing.T) {
	seq := OriginalFrequency(euro + dutch + us)
	t.Log("Original: ", seq)
	con := ConcurrentFrequencyWC([]string{euro, dutch, us})
	t.Log("Concurrent: ", con)
	if !reflect.DeepEqual(con, seq) {
		t.Fatal("ConcurrentFrequency wrong result")
	}
...
func BenchmarkConcurrentFrequencyWO(b *testing.B) {
	for i := 0; i < b.N; i++ {
		ConcurrentFrequencyWO([]string{euro, dutch, us})
	}
}

func BenchmarkConcurrentFrequencyWC(b *testing.B) {
	for i := 0; i < b.N; i++ {
		ConcurrentFrequencyWC([]string{euro, dutch, us})
	}
}
